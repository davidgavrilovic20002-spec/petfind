...
function setup({role='vet',scope='write',owner=false,signedIn=true,mfa=false,aal='aal1',revoked=false,queryError=false,walkIns=[],rpcResult='claimed-pet-id'}={}){
  const calls=[], inserts=[], rpcs=[];
  const client={rpc(name,args){rpcs.push({name,args});return Promise.resolve({data:rpcResult});},from(table){const filters=[];let inserted=null,one=false;const query={select(){return query;},eq(k,v){filters.push([k,v]);return query;},is(k,v){filters.push([k,v]);return query;},order(){return query;},limit(n){filters.push(['limit',n]);return query;},ilike(k,v){filters.push([k,v]);return query;},insert(row){inserted=row;return query;},then(resolve,reject){calls.push({table,filters});let data=[];
  ...
  const PFDB={client,getUser:async()=>signedIn?{id:'user'}:null,getProfile:async()=>({data:{id:'user',role}}),mfaAAL:async()=>({data:{currentLevel:aal,nextLevel:mfa?'aal2':aal}})};
...
test('read-only, revoked and owner contexts cannot use vet record writer',async()=>{
  for(const options of [{scope:'read',aal:'aal2'},{revoked:true,aal:'aal2'},{owner:true,role:'owner',aal:'aal2'}]){const {api,inserts}=setup(options);await assert.rejects(api.addRecord(petId,'vaccination',vaccination,'entry'));assert.equal(inserts.length,0);}
});
test('record insert binds author and pet to verified context and strips unexpected fields',async()=>{
  const {api,inserts}=setup({aal:'aal2'});await api.addRecord(petId,'vaccination',{...vaccination,administered_by:'attacker',pet_id:'other',official_synced:true},'stable-id');
...
test('a vet without a verified second factor cannot open a record, but can still see their list',async()=>{
  // 0035 gates the medical record on aal2. It deliberately does NOT gate the
  // patient list: the database does not, and locking a vet out of their own
  // caseload would enforce a rule that does not exist.
  const {api}=setup({aal:'aal1'});
  await assert.rejects(api.records(petId),err=>err.code==='mfa_setup_required');
  await assert.doesNotReject(api.caseload());
});
test('an owner reading their own pet is not held to the clinic 2FA rule',async()=>{
  const {api}=setup({role:'owner',owner:true,aal:'aal1'});
  await assert.doesNotReject(api.recordContext(petId));
});
...
test('caseload queries active grants for current vet',async()=>{const {api,calls}=setup();assert.equal((await api.caseload()).length,1);assert.deepEqual(calls[0].filters,[['vet_id','user'],['status','active'],['limit',200]]);});
test('caseload is bounded and says so when it is cut short',async()=>{
  // An unbounded list that quietly stops at a server default looks like a
  // permissions problem to the vet whose patient is missing.
  const {api}=setup();
